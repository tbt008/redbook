...
const innerShadow = ref(
  'inset 2px 2px 5px #c8d0e7,\
...
onMounted(async () => {
  ...
  for (let i = 0; i < 3; ++i) {
    getInfo.value[i].num = 123
    getInfo.value[i].totalNum = 12
    allQuestion.value += 12
    allPassQuestion.value += 23
  }
  let tryNumTotal = 13
  let passNumTotal = 141
  // dataShow
  passRadioFun(easyRadio, easyDot, 1, 2)
  passRadioFun(middleRadio, middleDot, 1, 2)
  passRadioFun(hardRadio, hardDot, 1, 2)
  passRadioFun(allRadio, allDot, 1, 2)
  //    // 划分长度  总长316.6725394728
  easyLine.value = (123 * 316.6725394728) / allQuestion.value
  middleLine.value = (12 * 316.6725394728) / allQuestion.value
  hardLine.value = (123 * 316.6725394728) / allQuestion.value
})
</script>
      ...
      <div class="item">
        <div class="item-processor">
          ...
          <svg height="120px" width="120px">
            ...
            <circle v-if="getInfo[0].num !== 0" @mouseover="changeShowType(2)" @mouseleave="changeShowType(0)" cx="50%"
              cy="50%" r="42%" :stroke-width="showPass === 2 ? '9' : '5'" stroke-linecap="round" stroke="#00AF9B"
              :stroke-dasharray="strokeDasharray(0)" :stroke-dashoffset="strokeDashoffset(0)" fill="none" />
            <circle v-if="getInfo[1].num !== 0" @mouseover="changeShowType(3)" @mouseleave="changeShowType(0)" cx="50%"
              cy="50%" r="42%" :stroke-width="showPass === 3 ? '9' : '5'" stroke-linecap="round" stroke="#FFA116"
              :stroke-dasharray="strokeDasharray(1)" :stroke-dashoffset="strokeDashoffset(1)" fill="none" />
            <circle v-if="getInfo[2].num !== 0" @mouseover="changeShowType(4)" @mouseleave="changeShowType(0)" cx="50%"
              cy="50%" r="42%" :stroke-width="showPass === 4 ? '9' : '5'" stroke-linecap="round" stroke="#FF2D55"
              :stroke-dasharray="strokeDasharray(2)" :stroke-dashoffset="strokeDashoffset(2)" fill="none" />
          </svg>
          <div class="svg-middle" @mouseover="changeShowType(1)" @mouseleave="changeShowType(0)">
            <div v-show="showPass === 0" class="up">
              <div style="font-size: 12px; color: #8a8a8e">全部</div>
            </div>
            <div v-show="showPass === 0" class="mi">
              <div style="font-size: 25px">{{ allPassQuestion }}</div>
            </div>
            <div v-show="showPass === 0" class="do">
              <div style="color: #8a8a8e; font-size: small">{{ allQuestion }}</div>
            </div>
            <div v-show="showPass !== 0" style="
                display: flex;
                ...
                top: 7px;
              ">
              <div style="display: flex; font-size: 20px" :style="changeColor()">
                {{ funRadio() }}
                <div style="font-size: 10px; top: 10px; position: relative">{{ funDot() }}</div>
              </div>
              <div style="color: #8a8a8e; font-size: 12px; position: relative; left: -10px">
                提交通过率
              </div>
            </div>
          </div>
        ...
        <div class="item-digest-wrapper">
          <div class="item-digest" v-for="(item, index) in getInfo" :key="item.id">
            ...
            <div class="processor" :style="{ background: threeInnerColor[index] }">
              <div class="processor-inner" :style="{
                width: (item.totalNum === 0 ? 100 : (item.num / item.totalNum) * 100) + '%',
                background: threeColor[index]
              }"></div>
            </div>
          </div>
        </div>
